...
class UserService {
  ...
  async create({ username, email, password, role, profile, status }) {
    ...
    user.setEmail(email);
    user.setPassword(password); //not hashed because parse server hash it automatically 
    user.set("role", role);
  ...
  async update(id, updateData) {
    try {
      ...
      if (
        ...
        !UserStatus.isValid(updateData.status)
      ) {
        return {
          success: false,
      ...
      if (
        ...
        !UserRoles.isValid(updateData.role)
      ) {
        return {
          success: false,
      ...
      if (
        error.code === Parse.Error.VALIDATION_ERROR ||
